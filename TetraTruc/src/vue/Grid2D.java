...
public class Grid2D implements GridObserver{
	...
	public void draw(Graphics g, int width, int height){
		...
		for(int i=0; i<4; ++i){
			if(this.nextShapes != null)
				grid[i][i].draw(g, theme.getColorByShape(nextShapes[i]), nextShapes[i].getPoint(i).getX(), nextShapes[i].getPoint(i).getY(), this.originNextShapeLeft + offsetX, this.originNextShapeTop + offsetY, 0.8f);	// Modifie la couleur de la brique
		}
